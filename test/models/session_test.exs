defmodule Xee3rd.SessionTest do
  use Xee3rd.ModelCase
  use Xee3rd.ConnCase

  alias Xee3rd.User
  alias Xee3rd.Session
  ...
  setup do
    ...
    User.create(changeset, Xee3rd.Repo)
  ...
  test "check whether loggedin or not" do
    conn = conn()
    session = %{"name" => @correct_attrs[:name], "password" => @correct_attrs[:password]}
    {_status, user} = Session.login(session, Xee3rd.Repo);
